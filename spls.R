        ...
        scale.x=TRUE, scale.y=FALSE, eps=1e-4, maxstep=100, trace=FALSE )
{
	...
	#DEBUG
	print(paste("[spls]k=",k,", length(what)=",length(what),", length(what!=0)=",length(which(what!=0)),sep=""))
	#print("what[which(what!=0)]")
	#print(what[which(what!=0)])
	...
	#DEBUG
	#print(paste("length(A) ",length(A),sep=""))
	#print("dim(pj)")
	#print(dim(pj))
	#print("pj")
	#print(pj[1:5, ])
    ...
    object <- list( x=x, y=y, betahat=betahat, A=A, betamat=betamat, ws=ws, new2As=new2As,
