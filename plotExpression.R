...
pdf("../data/spca7_kernel_pcer0.01/expression_tc.pdf")
par(mfrow=c(3, 2), oma=c(0,0,2,0))

print("rownames(regen_c)")
print(rownames(regen_c)[tpoints_c])
for (i in 1:4) {
	
	#Extract genes to plot
	genesToPlot = readLines(paste("../data/spca7_kernel_pcer0.01/PC",i,".txt",sep=""))
	print("length(genesToPlot)")
	print(length(genesToPlot))
	
	genesToPlotT = regen_t[tpoints_t, which(colnames(regen_t) %in% genesToPlot)]
	genesToPlotC = regen_c[tpoints_c, which(colnames(regen_c) %in% genesToPlot)]
	foldChange = log10(genesToPlotT / genesToPlotC)
	foldChange[is.na(foldChange)] = 0
	#print("dim(genesToPlotT)")
	#print(dim(genesToPlotT))
	print("dim(genesToPlotC)")
	print(dim(genesToPlotC))

	matplot(foldChange, cex=1, pch=16, type="l", xaxt="n", xlab ="Timepoints", ylab="Selected gene expression", main = paste("V_", i, sep=" "), cex.lab=1.25, cex.axis=1.25)
	axis(1, at=1:8, labels=rownames(regen_c)[tpoints_c])
}
