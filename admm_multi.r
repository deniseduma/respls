...
#ADMM splitting by features with lasso trick but calling ADMM instead of glmnet locally

library("rdetools")
library("multicore")

source("admm_lasso_multi.r")
...
admm <- function(X, K, Lapl, S, alpha, init_beta=NULL, p.ini, sselection, pcer, thr.range, sampleNo, sampleSize, savepath) {
	...
	for(l in 1:length(lambdas)){
		...
		decomps <- vector("list", NCORES)
		for (j in 1:NCORES) {
			res<-factor(newX[, ((j-1)*SSIZE+1):(j*SSIZE)], rho) 
			decomps[[j]]<-list(L=res$L, U=res$U)
		}

		#modified l1 penalty
		lam1 <- lam1 / sqrt(1+lam2)
	
		temp <- sss(newX,y,beta,decomps,lam1,lam2,rho,relaxpar,sselection,sampleNo,sampleSize)
...
sss <- function(A,b,xold,decomps,lam1,lam2,rho,relaxpar,sselection,sampleNo,sampleSize) {
	if (sselection) {
		...
		res <- admm_lasso_multi(A,b,xold,decomps,lam1,rho,relaxpar,sselection)
	}
	return(res)
}
