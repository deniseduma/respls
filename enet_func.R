    ...
    trace = FALSE, eps = .Machine$double.eps) 
{
    ...
    while ((k < max.steps) & (length(active) < maxvars)) {
        ...
        u <- c(u1, u2)
        #DEBUG5
	print("[debug5]length(w)")
	print(length(w))
	print("[debug5]length(u1)")
	print(length(u1))
	print("[debug5]length(u2)")
	print(length(u2))
