...
regression_graph <- function(X,L,alpha,init_beta,lam1,lam2,tol,sselection) {
	...
	if (sselection==F) {
		k <- 1
		obj <- numeric()
		obj[k] <- reg_obj(y,X,beta,L,lam1,lam2)
	}

	diff <- 1
	while (diff>tol) {
		g <- gprox(y,X,gamma,L,lam1,lam2,t)
		diff <- sqrt(sum(g^2))
		#print(paste("[regression_graph]diff=", diff, sep=""))
		new_beta <- gamma - t * g
		gamma <- new_beta + (k-1)/(k+2) * (new_beta - beta)
		beta <- new_beta
		if (sselection==F) {
			k <- k + 1
			obj[k] <- reg_obj(y,X,beta,L,lam1,lam2)
	...
	print(paste("[regression_graph]k=", k,", t=",t,sep=""))
	
	#if (sselection==F) {
	#	dev.new()
	#	plot(x<-1:k, y<-log10(obj), type="p", cex=1, col="red", xlab="iteration", ylab="objective", xlim=c(1,k), main=paste("lambda ", lam1, sep=""))
		#Sys.sleep(0.5) 
		#dev.off()
	#}
