	...
	for(k in 1:nbiclust){
		...
		if(pointwise){
			for(i in 1:iter){
				...
				vc <- updatev.ns(X,u1,pcerv,n.ini,ss.thr,steps,size,gamm,cols.nc,vc$l)
			...
			for(i in 1:iter){
				...
				vc <- updatev.ns(X,u1,pcerv,n.ini,ss.thr,steps,size,gamm,cols.nc,savepath)
				v1 <- vc[[1]]/sqrt(sum(vc[[1]]^2)) 
				v1[is.na(v1)] <- 0
				if(uc[[3]] & i > start.iter){
					cat("rows not stable")
...
updatev.ns <- function(X,u0,pcer,n.ini,ss.thr,steps,size,gamm,cols.nc=FALSE,savepath=FALSE){
	n.ini  <-	n <- ncol(X)
	ols <- t(X)%*%u0	
	stop <- FALSE
	...
	return(list(vc=vc,sp=NULL,stop=stop,qs=NULL,thr=NA,l=NA,delta=NA))
}
...
sampleTPoints <- function(s) {
	count = 0
	index = numeric(8)
	for (i in c(1, 3, 5, 7, 9, 11, 13, 15)) {
		count = count + 1
		index[count] = sample(c(i, i+1), size=1)
	}
	return(index) 
}
...
adaLasso.nc <- function(X,b,lambda,steps,size,gamm=0){
	...
	subsets <- sapply(1:steps, function(s) {sampleTPoints(s)})
