...
admm <- function(X, K, Lapl, S, alpha, init_beta=NULL, p.ini, sselection, pcer, thr.range, sampleNo, sampleSize, savepath) {
	...
	for(l in 1:length(lambdas)){
		...
		temp <- sss(newX,y,beta,L,U,lam1,lam2,rho,relaxpar,sselection,sampleNo,sampleSize)
...
sss <- function(A,b,x_init,L,U,lam1,lam2,rho,relaxpar,sselection,sampleNo,sampleSize) {
	if (sselection) {
		...
		res <- admm_lasso(A,b,x_init,L,U,lam1,rho,relaxpar,sselection)
	}
	return(res$x)
}
