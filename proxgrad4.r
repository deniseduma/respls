#Accelerated gradient with line search
...
regression_graph <- function(X,L,alpha,init_beta,lam1,lam2,tol,sselection) {
	...
	while (diff>tol) {
		...
		while (TRUE) {
			...
			new_beta <- gamma - t * g
			new_gamma <- new_beta + (k2-1)/(k2+2) * (new_beta - beta)
			new_obj <- smth_obj(y,X,new_beta,L,lam1,lam2)
			crt_obj <- smth_obj(y,X,gamma,L,lam1,lam2) 
			crt_smth_grad <- smth_grad(y,X,gamma,L,lam1,lam2)
			rhs <- crt_obj - t * t(crt_smth_grad) %*% g + (t/2) * sum(g^2)
